...
func Test_application_handlers(t *testing.T) {
	...
	for _, e := range theTests {
		...
		if resp2.StatusCode != e.expectedFirstStatusCode {
			t.Errorf("%s: expected first returned status to be %d, but got %d", e.name, e.expectedFirstStatusCode, resp2.StatusCode)
		}
	}
}
...
func TestAppHome(t *testing.T) {
	...
	for _, e := range tt {
		...
		if rr.Code != http.StatusOK {
			t.Errorf("TestAppHome returned wrong status code; expected 200 and got %d", rr.Code)
		}

		body, _ := io.ReadAll(rr.Body)
		if !strings.Contains(string(body), e.expectedHTML) {
			t.Errorf("%s: did not find %s correct text in html", e.name, e.expectedHTML)
		}
	}
}

func TestApp_renderWithBadTemplate(t *testing.T) {
	pathToTemplatesBak := pathToTemplates
	...
	err := app.render(rr, req, "bad.page.tpl", &TemplateData{})
	if err == nil {
		t.Error("expected error from bad template, but did not get one")
	}

	pathToTemplates = pathToTemplatesBak
}
...
func Test_app_Login(t *testing.T) {
	...
	for _, e := range tt {
		...
		if actualLoc.String() != e.expectedLoc {
			t.Errorf("%s: expected location %s but got %s", e.name, e.expectedLoc, actualLoc)
		}
	}
}
