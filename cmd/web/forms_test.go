...
func TestForm_Has(t *testing.T) {
	...
	has = form.Has("a")
	if !has {
		t.Error("shows gorm not gave field when it should")
	}
}
...
func TestForm_Required(t *testing.T) {
	...
	if !form.Valid() {
		t.Error("shows post does not have required fields, when it dows")
	}
}
...
func TestForm_ErrorGet(t *testing.T) {
	...
	if len(s) == 0 {
		t.Error("should have an error returned form Get, but do not")
	}
